...
function App() {
  ...
  return (<Stack className="App" sx={{top: 0}}>
    <Header items={items} setItems={setItems} handleDeleteAll={handleDialogConfirmDeleteAll} handleToggle={handleToggle} />
      {items.length === 0 ? <Box sx={{height: 'calc(100vh - 60px - 60px)', width: '100%', maxWidth: '800px', display: 'flex', justifyContent: 'center', alignItems: 'center', backgroundColor: 'white'}}>
        <Typography variant="h5">No items added.</Typography>
      </Box> : 
      <Stack className="List" sx={{width: '100vw', maxWidth: '800px', alignItems: 'center', backgroundColor: 'white', zIndex: 1, height: 'calc(100vh - 60px - 60px)'}}>
